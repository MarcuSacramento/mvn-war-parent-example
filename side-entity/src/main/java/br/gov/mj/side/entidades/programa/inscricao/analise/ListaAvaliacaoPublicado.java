...
@Entity
@Table(name = "tb_lap_lista_avaliacao_publicado", schema = "side")
public class ListaAvaliacaoPublicado extends BaseEntity<Long> {

    private static final long serialVersionUID = 1L;

    @Id
    @Column(name = "lap_id_lista_avaliacao")
    @SequenceGenerator(name = "tb_lap_lista_avaliacao_publicado_generator", sequenceName = "side.seq_tb_lap_lista_avaliacao_publicado", allocationSize = 1)
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "tb_lap_lista_avaliacao_publicado_generator")
    @NotNull
    private Long id;

    @Column(name = "lap_no_nome_arquivo")
    @NotNull
    ...
    @Column(name = "lap_tp_tipo_lista")
    @NotNull
    private EnumTipoLista tipoLista;

    @ManyToOne
    @JoinColumn(name = "lap_fk_prg_id_programa")
    @NotNull
    private Programa programa;

    @Column(name = "lap_ct_conteudo_anexo")
    @Type(type = "org.hibernate.type.BinaryType")
    @NotNull
    private byte[] conteudo;

    @Type(type = "org.jadira.usertype.dateandtime.threeten.PersistentLocalDateTime")
    @Column(name = "lap_dt_data_cadastro_documento")
    @NotNull
    private LocalDateTime dataCadastro;

    @Column(name = "lap_no_usuario_cadastro")
    @NotNull
    private String usuarioCadastro;

    @Column(name = "lap_vl_tamanho_anexo")
    @NotNull
    private Long tamanho;

    public ListaAvaliacaoPublicado() {
    }
