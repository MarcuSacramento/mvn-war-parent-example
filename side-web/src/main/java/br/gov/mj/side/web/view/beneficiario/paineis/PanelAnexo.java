...
public class PanelAnexo extends Panel {
	...
	public PanelAnexo(String id, Page backPage, List<EntidadeAnexo> listAnexoTemp,EntidadeDto entidade,boolean readOnly) {
		super(id);
	...
	public void initVariaveis() {
		...
		if (entidade.getId() != null) {
			listAnexoTemp = SideUtil.convertAnexoDtoToEntityEntidadeAnexo(anexoService.buscarPeloIdEntidade(entidade.getId()));
		}
		if (uploads == null) {
	...
	public DropDownChoice<EnumTipoArquivoEntidade> getDropDownTipo() {
		...
		lista.add(EnumTipoArquivoEntidade.COMPROVANTE_ENDERECO);
		lista.add(EnumTipoArquivoEntidade.DEFINICAO_COMPETENCIA_SERVIDOR);
		lista.add(EnumTipoArquivoEntidade.DOCUMENTACAO_PESSOAL);
		lista.add(EnumTipoArquivoEntidade.NORMA_INSTITUTIVA);
	...
	private InfraLocalDateTextField newDateTextFieldDataAnexo() {
		InfraLocalDateTextField field = componentFactory.newDateTextFieldWithDatePicker("data", "Data", false, new PropertyModel<LocalDate>(this, "dataAnexo"), "dd/MM/yyyy",
				"pt-BR");
	...
	private DataView<EntidadeAnexo> getDataViewAnexos(String id) {
		...
		DataView<EntidadeAnexo> dataView = new DataView<EntidadeAnexo>(id, new EntityDataProvider<EntidadeAnexo>(listAnexoTemp)) {
			private static final long serialVersionUID = 1L;
...
/*		BeneficiarioPage pagina = (BeneficiarioPage) backPage;
